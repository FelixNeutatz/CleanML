...
for k, v in pointer_dict.items():
    ...
    fig, axs = plt.subplots(np.sum(np.array(feat_type) == 'Numerical'))
    axi = 0
    feature_names = ''
    skew = ''
    for fi in range(len(feat_type)):
        if feat_type[fi] == 'Numerical':
            feature_names += str(X.columns[fi]) + ','

            if dataset == 'eeg' and error_type=='outliers':
                axs[axi].hist(X.values[:,fi], 50, density=True, facecolor='g', alpha=0.75)
                axs[axi].set_title('Histogram of ' + str(X.columns[fi]))
                axi +=1

            skew += str(scipy.stats.skew(X.values[:,fi])) + ','
    plt.grid(True)
    plt.show()
