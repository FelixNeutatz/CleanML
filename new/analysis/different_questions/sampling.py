...
def get_hyperparameter_values(mypath):
    ...
    check_components = ["'data_preprocessing:numerical_transformer:imputation:training_fraction':",
                        "'data_preprocessing:categorical_transformer:imputation:training_fraction':",
                        "'data_preprocessing:numerical_transformer:outlier_detection:detection:training_fraction':"
                        ]
    ...
    for i in range(len(results['models'])):
        ...
        for check_i in range(len(check_components)):
            count_models_in_ensemble[check_components[check_i]] = 0.0
        count_models = 0
        ...
        for model_i in range(len(all_models_with_weights)):
            ...
            count_models += 1.0#current_weihgt
            #print(pipeline)
            for check_i in range(len(check_components)):
                if check_components[check_i] in str(pipeline):
                    new_val = float(str(pipeline).split(check_components[check_i])[-1].split(',')[0])
                    count_models_in_ensemble[check_components[check_i]] += new_val

        for k, v in count_models_in_ensemble.items():
            count_classifiers[k].append(v / float(count_models))
        print(task_name + ': ' + str(count_classifiers))
